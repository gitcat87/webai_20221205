...
var db_host = 'localhost';//'192.168.0.80';//'localhost';
var db_port = 1521;
var db_name = 'xe';
var db_id   = 'kb';
var db_pwd  = 'kb602';
...
_app.get('/d3/:category', function(req,res){
  ...
  if(_q.work == '00'){
    ...
    var _query = `
    SELECT TO_CHAR(KBAN_PERIOD_STRT, 'DAY') AS DAY_OF_WEEK, COUNT(*) AS RECORD_COUNT
    FROM KB.GYG_KANBAN
    WHERE KBAN_PERIOD_STRT >= DATE '${_q.begin}' AND KBAN_PERIOD_STRT <= DATE '${_q.end}'
    GROUP BY TO_CHAR(KBAN_PERIOD_STRT, 'DAY'), TO_CHAR(KBAN_PERIOD_STRT, 'D')
    ORDER BY TO_CHAR(KBAN_PERIOD_STRT, 'D')
    `;
    console.log(_query);
    ExcuteOracle(db_host,db_port,db_name, db_id, db_pwd, res, _query);
  }  else if(_q.work == '03'){
